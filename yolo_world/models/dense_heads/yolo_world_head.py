...
class YOLOWorldHead(YOLOv8Head):
    ...
    def loss(self, img_feats: Tuple[Tensor], txt_feats: Tensor,
        ...
        # Force class count to match current text prompts
        if txt_feats is not None:
            self.num_classes = txt_feats.shape[1]
            self.head_module.num_classes = txt_feats.shape[1]
    ...
    def loss_by_feat(
        ...
        if batch_text_masks is not None:
            self.num_classes = batch_text_masks.shape[1]
            self.head_module.num_classes = batch_text_masks.shape[1]
